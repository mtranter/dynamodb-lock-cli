...
func createTable(client *dynamolock.Client, tableName string) error {
	...
	if err != nil {
		...
		isTableAlreadyCreatedError := xerrors.As(err, &awsErr) && awsErr.StatusCode() == 400 && strings.Contains(awsErr.Message(), "Table already exists")
		if !isTableAlreadyCreatedError {
			return xerrors.Errorf("cannot create dynamolock client table: %w", awsErr.Message())
		}
	}
	return nil
}
